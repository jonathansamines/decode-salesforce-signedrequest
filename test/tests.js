var expect = chai.expect;
var decode = signedRequest.decode;

var signed_request = "V/lsK06wzoefsXdKVc231sIkTxi5zYV2J2FW1hlu0VQ=.";
var invalid_signed_request = "bla-bla-invalid_signed_request";

describe('positive and -ve tests', function() {
	it('Should properly decode', function() {
		var result = decode(signed_request);

		expect(result.client).to.be.ok;
		expect(result.client.oauthToken).to.be.ok;
		expect(result.client.instanceUrl).to.be.ok;
		expect(result.context.environment.parameters).to.be.ok;
	});

	it('should say: Must must pass signedRequest string.', function() {
		var result = decode();
		expect(result.message).to.equal('Must pass the signedRequest string.');
	});

	it('Should should not be able to parse signed-request', function() {
		var result = decode(invalid_signed_request);
		expect(result.message).to.equal("Could not parse signed-request.");
	});
})
